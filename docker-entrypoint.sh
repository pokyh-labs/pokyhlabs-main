...
mkdir -p /app/data /app/backend/uploads /app/backend/logs /root/.cloudflared

# Hand ownership to appuser so the app (non-root) can write to these dirs.
chown appuser:appgroup /app/data /app/backend/uploads /app/backend/logs /root/.cloudflared
