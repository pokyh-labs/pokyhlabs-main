...
// ── Workflow statuses ─────────────────────────────────────────────────────────
const STATUSES = [
  { id: 'new',         label: 'Neu',               color: '#593df8', bg: 'rgba(89,61,248,0.08)', border: 'rgba(89,61,248,0.2)' },
  { id: 'in_progress', label: 'Wird bearbeitet',   color: '#2563eb', bg: 'rgba(37,99,235,0.08)', border: 'rgba(37,99,235,0.2)' },
  { id: 'developing',  label: 'Wird programmiert', color: '#7c3aed', bg: 'rgba(124,58,237,0.08)', border: 'rgba(124,58,237,0.2)' },
  { id: 'waiting',     label: 'Warten auf Kunde',  color: '#d97706', bg: 'rgba(217,119,6,0.08)', border: 'rgba(217,119,6,0.2)' },
  { id: 'done',        label: 'Erledigt',          color: '#28a745', bg: 'rgba(40,167,69,0.08)', border: 'rgba(40,167,69,0.2)' },
  { id: 'archived',    label: 'Archiviert',        color: '#8c8c8c', bg: 'rgba(0,0,0,0.04)',     border: 'rgba(0,0,0,0.1)' },
];
const META = Object.fromEntries(STATUSES.map(s => [s.id, s]));
function meta(id) {
  return META[id] || { id, label: id || '—', color: '#8c8c8c', bg: 'rgba(0,0,0,0.04)', border: 'rgba(0,0,0,0.1)' };
}
...
export default function Inquiries() {
  ...
  const load = useCallback(async (status) => {
    ...
    try {
      ...
      toast('Anfragen konnten nicht geladen werden', 'error');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function setStatus(id, status) {
    ...
    try {
      ...
      toast(`Markiert als „${meta(status).label}“`);
    } catch {
    ...
    <div style={{ display: 'flex', gap: '1.25rem', height: 'calc(100vh - 120px)', minHeight: 500 }}>
      ...
      <div style={{ width: selected ? '44%' : '100%', transition: 'width 0.25s', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>{total} gesamt</span>
        </div>

        {/* Filter tabs */}
        <div style={{ display: 'flex', gap: 5, marginBottom: '0.875rem', flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${active ? 'rgba(0,0,0,0.12)' : 'rgba(0,0,0,0.06)'}`,
                  borderRadius: 8, padding: '5px 12px', cursor: 'pointer',
                  color: active ? 'var(--text)' : 'var(--text3)',
                  fontSize: '0.78rem', fontWeight: active ? 600 : 400,
                  boxShadow: active ? '0 1px 3px rgba(0,0,0,0.06)' : 'none', transition: 'all 150ms',
                }}
              >
                {f === 'all' ? 'Alle' : meta(f).label}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: '#fff', border: '1px solid var(--border)', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 10 }}>
              ...
              <p style={{ color: 'var(--text3)', fontSize: '0.82rem', margin: 0 }}>Keine Anfragen gefunden</p>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Name', 'Services', 'Status', 'Datum', ''].map(h => (
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        ...
                        background: isActive ? 'rgba(89,61,248,0.04)' : 'transparent', transition: 'background 150ms',
                      }}
                    ...
                    >
                      <td style={{ padding: '12px 14px', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          {inq.status === 'new' && <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0 }} />}
                          <div style={{ fontWeight: inq.status === 'new' ? 700 : 500, fontSize: '0.83rem', color: 'var(--text)', letterSpacing: '-0.01em' }}>
                            {inq.name}
                          </div>
                        </div>
                        <div style={{ fontSize: '0.73rem', color: 'var(--text3)', marginTop: 2, marginLeft: inq.status === 'new' ? 14 : 0 }}>{inq.email}</div>
                      </td>
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          ...
                          {inq.services.length > 2 && (
                            <span style={{ fontSize: '0.71rem', color: 'var(--text3)', padding: '2px 4px' }}>+{inq.services.length - 2}</span>
                          )}
                        </div>
                      </td>
                      <td style={{ padding: '12px 14px', whiteSpace: 'nowrap' }}>
                        <span style={{ background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, borderRadius: 6, padding: '3px 9px', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.02em' }}>
                          {sc.label}
                        </span>
                      </td>
        ...
        <div style={{ flex: 1, minWidth: 0, background: '#fff', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: '#fafafa' }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: '0.95rem', fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em' }}>{selected.name}</div>
              <a href={`mailto:${selected.email}`} style={{ fontSize: '0.75rem', color: 'var(--text3)', marginTop: 2, textDecoration: 'none', display: 'inline-block' }}>{selected.email}</a>
            </div>
            <button
              onClick={() => setSelected(null)}
              style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', color: 'var(--text3)', fontSize: '0.75rem', transition: 'all 150ms', flexShrink: 0 }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
            ...
            <DetailSection label="Status">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {STATUSES.map(s => {
                  ...
                  const busy = updating === selected.id;
                  return (
                    <button
                      key={s.id}
                      ...
                      style={{
                        ...
                        transition: 'all 150ms', opacity: busy && !active ? 0.5 : 1,
                      }}
                    >
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: s.color, opacity: active ? 1 : 0.4 }} />
                      {s.label}
                    </button>
                  );
                })}
            ...
            {selected.company && (
              <DetailSection label="Unternehmen">
                <span style={{ fontSize: '0.9rem', color: 'var(--text)', fontWeight: 500 }}>{selected.company}</span>
              </DetailSection>
            )}

            <DetailSection label="Nachricht">
              {selected.description ? (
                <p style={{ fontSize: '0.875rem', color: 'var(--text2)', lineHeight: 1.75, margin: 0, whiteSpace: 'pre-wrap' }}>
                  {selected.description}
                </p>
              ) : (
                <span style={{ fontSize: '0.82rem', color: 'var(--text3)' }}>Keine Nachricht angegeben</span>
              )}
            </DetailSection>

            {selected.deadline && (
              <DetailSection label="Deadline">
                <span style={{ fontSize: '0.9rem', color: '#d97706', fontWeight: 600 }}>{fmtDate(selected.deadline)}</span>
              </DetailSection>
            )}

            <DetailSection label="Eingegangen">
              <span style={{ fontSize: '0.85rem', color: 'var(--text2)' }}>{fmt(selected.createdAt)}</span>
            </DetailSection>
          </div>

          {/* Detail footer */}
          <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', flexShrink: 0, background: '#fafafa' }}>
            <button
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: '0.83rem', color: 'var(--text3)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              Von <strong style={{ color: 'var(--text)' }}>{confirmDelete.name}</strong> ({confirmDelete.email}). Das kann nicht rückgängig gemacht werden.
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              >
                {deleting === confirmDelete.id ? 'Löschen…' : 'Löschen'}
              </button>
            </div>
          </div>
        </div>
